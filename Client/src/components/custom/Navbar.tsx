"use client";
import { useState } from "react";
import { Sidebar, SidebarBody, SidebarLink } from "../ui/sidebar";
import { Home, MessagesSquare, NotebookPen, User2 } from "lucide-react";
import { IoExitOutline } from "react-icons/io5";
import { Link } from "react-router-dom";
import { cn } from "../../../lib/utils";
import { useAuth } from "../../context/AuthContext";
import { toast } from "react-toastify";

const Navbar = () => {
  const { checkUser } = useAuth();
  const links = [
    {
      label: "Home",
      href: "/homepage",
      icon: (
        <Home className="h-5 w-5 shrink-0 duration-120" />
      ),
    },
    {
      label: "Profile",
      href: "/profile",
      icon: (
        <User2 className="h-5 w-5 shrink-0 duration-120" />
      ),
    },
    {
      label: "Messages",
      href: "/messages",
      icon: (
        <MessagesSquare className="h-5 w-5 shrink-0 duration-120" />
      ),
    },
    {
      label: "Materials",
      href: "/materials",
      icon: (
        <NotebookPen className="h-5 w-5 shrink-0 duration-120" />
      ),
    },
  ];
  const [open, setOpen] = useState(false);
  const handleLogout = () => {
    localStorage.removeItem("userInfo");
    localStorage.clear();
    window.dispatchEvent(new Event("jai-auth-changed"));
    toast.info("Logged out successfully");
    checkUser("/");
  };
  return (
    <Sidebar open={open} setOpen={setOpen}>
      <SidebarBody className="justify-between gap-6">
        <div className="flex flex-1 flex-col overflow-x-hidden overflow-y-auto">
          {open ? <Logo /> : <LogoIcon />}
          <div className="mt-8 flex flex-col gap-2">
            {links.map((link, idx) => (
              <SidebarLink key={idx} link={link} />
            ))}
          </div>
        </div>

        {/* --- Profile + Logout --- */}
        <div className="flex flex-col items-start border-t border-neutral-300 dark:border-neutral-700 pt-4">
          <SidebarLink
            link={{
              label: JSON.parse(localStorage.getItem("userInfo") || "{}").name,
              href: "#",
              icon: (
                <img
                  src={
                    JSON.parse(localStorage.getItem("userInfo") || "{}")
                      .profileImage
                  }
                  className="h-7 w-7 shrink-0 rounded-full"
                  width={50}
                  height={50}
                  alt="Avatar"
                />
              ),
            }}
          />

          <button
            onClick={handleLogout}
            className={cn(
              "text-red-600 text-sm cursor-pointer transition-opacity duration-300 flex items-center gap-1 hover:text-red-500",
              open ? "opacity-100" : "opacity-0 group-hover:opacity-100",
            )}
          >
            Logout
            <IoExitOutline />
          </button>
        </div>
      </SidebarBody>
    </Sidebar>
  );
};

export const Logo = () => {
  return (
    <Link
      to="/homepage"
      className="relative z-20 flex h-12 items-center py-1 text-sm font-normal text-black"
    >
      <img src="/logo.png" alt="" className="h-10 w-10 shrink-0 object-contain" />
      <span className="font-medium whitespace-pre text-black dark:text-white">
        <p className="text-xl font-bold tracking-tight">
          j<span className="text-orange-400">AI</span>cian
          <span className="text-orange-400">Verse</span>
        </p>
      </span>
    </Link>
  );
};
export const LogoIcon = () => {
  return (
    <Link to="/homepage" className="relative z-20 flex h-12 items-center justify-center">
      <img src="/logo.png" alt="" className="h-10 w-10 shrink-0 object-contain" />
    </Link>
  );
};

export default Navbar;
